...
## 有料版・本への案内
- ここに入っているのは「そのまま置ける雛形」だけです。「なぜこの4つを人に残すのか」「hooks をどう設計するか」「実際に起きた事故の型と防ぎ方」までまとめた解説は、有料の設定パック（BOOTH、tsunagio）にあります。
- 実例つきでもう一段深く作り込みたい場合は、Zenn の本（tsunagio）も参考にしてください。
